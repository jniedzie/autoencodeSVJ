...
signals = {"mZprime{}_mDark20_rinv{}_alphapeak".format(mass, rinv).replace(".", ""): "{}{}GeV_{:1.2f}/base_3/*.h5".format(config.signals_base_path, mass, rinv)
           for mass in masses
           for rinv in rinvs}
...
def get_hist_data(events, event_indices, jets, errors):
    ...
    for iEvent in event_indices:
        n_events += 1
        if n_events > config.n_events_per_class:
            break
        ...
        if loss_jet_1 > config.svj_jet_cut:
            n_svj_jets += 1
        
        if loss_jet_2 > config.svj_jet_cut:
            n_svj_jets += 1
...
output_file = ROOT.TFile(config.stat_hists_path, "recreate")
output_file.cd()
